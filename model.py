...
class FRITNet(nn.Module):
    def __init__(self, num_classes=7, transformer_depth=2, use_srt=False):
        super().__init__()
        ...
        self.classifier = nn.Linear(128, num_classes)  # unused if using transformer's head

    def forward(self, x, training=True):
        x = self.backbone(x)
        x = self.lfa(x)
        x = self.safm(x)
        logits, cls_out, aux_g, aux_l = self.transformer(x, training=training)
        return logits, cls_out, aux_g, aux_l