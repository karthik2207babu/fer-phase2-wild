...
# --- Configuration ---
BATCH_SIZE = 64
EPOCHS = 40                 # Reduced epochs since the model is already pre-trained
LEARNING_RATE = 5e-5        # Lower learning rate to prevent destroying pre-trained weights
...
# UPDATED: Paths for fine-tuning
SAVE_DIR = "/content/drive/MyDrive/FER_Phase2_FineTune"
PRETRAINED_WEIGHTS = "/content/drive/MyDrive/AffectNet_Phase1_Pretrain/affectnet_phase1_epoch_28.pth"
...
def train():
    ...
    print(f"Ready! Training on {len(train_dataset)} RAF-DB images.")

    # Initialize model with 7 classes for RAF-DB
    model = FRITNet(num_classes=7).to(device)

    # ==========================================
    # LOAD AFFECTNET WEIGHTS WITH FILTERING
    # ==========================================
    if os.path.exists(PRETRAINED_WEIGHTS):
        print(f"Loading AffectNet weights from: {PRETRAINED_WEIGHTS}")
        checkpoint = torch.load(PRETRAINED_WEIGHTS, map_location=device)
        model_dict = model.state_dict()
        
        # Filter out mismatched classification heads (8 classes vs 7 classes)
        filtered_checkpoint = {k: v for k, v in checkpoint.items() 
                               if k in model_dict and model_dict[k].shape == v.shape}
        
        model_dict.update(filtered_checkpoint)
        model.load_state_dict(model_dict)
        print(f"Successfully loaded {len(filtered_checkpoint)}/{len(model_dict)} parameter blocks.")
    else:
        print("WARNING: AffectNet weights not found. Check the path!")

    # Set alpha lower to let the classification heads catch up to the pre-trained features
    ...
    # Backbone gets an extremely low learning rate during fine-tuning
    ...
    log_file_path = os.path.join(SAVE_DIR, "finetune_log.txt")
    ...
    for epoch in range(EPOCHS):
        ...
        if v_acc > best_val_acc:
            best_val_acc = v_acc
            weights_path = os.path.join(SAVE_DIR, "best_finetuned_weights.pth")
    ...
    plot_path = os.path.join(SAVE_DIR, "finetune_plot.png")
